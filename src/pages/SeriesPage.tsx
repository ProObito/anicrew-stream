...
const SeriesPage = () => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["mm-series-list"],
    queryFn: async () => {
      const pages = await Promise.all([1, 2, 3, 4, 5, 6].map((p) => mm.tvshows(p).catch(() => null)));
      const items: any[] = [];
      for (const p of pages) {
        items.push(...pickArray(p, "tvshows", "shows", "items", "results"));
  ...
  return (
    <div className="pt-20 pb-24 md:pb-10 px-4 md:px-16 lg:px-24">
      ...
      {isError && <p className="text-muted-foreground">Failed to load series.</p>}
      {data && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {data.map((m) => (
            <MMCard key={m.slug} item={m} kind="tv" />
          ))}
        </div>
      )}
    </div>
